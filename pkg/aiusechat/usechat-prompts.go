...
var SystemPromptText = strings.Join([]string{
	...
	`When positioning widgets, you can use split_direction (horizontal/vertical) with target_widget and position (before/after) to control layout.`,
	`When moving widgets, specify the widget to move, the target widget, and whether to place it before/after and use horizontal/vertical split.`,
}, " ")
...
var SystemPromptText_OpenAI = strings.Join([]string{
	...
	// Remote files limitation
	`You cannot read/write remote files - if the user asks, say that feature isn't available yet but is coming soon.`,
